...
void MatrixConnection::processNewRoom(QMatrixClient::Room *room)
{
    ...
    if (room->memberCount() != 2) {
        // Multiuser room
        uint handle = ensureHandle(room);

        QTimer::singleShot(2000, this, [this,handle,room]() {
            prefetchRoomHistory(room, Tp::HandleTypeRoom, handle);
        });
        return;
    }
...
//    QTimer::singleShot(500, this, [this,handle,room]() {
//        prefetchRoomHistory(room, Tp::HandleTypeContact, handle);
//    });
    return handle;
}
...
void MatrixConnection::prefetchRoomHistory(QMatrixClient::Room *room, Tp::HandleType type, uint handle)
{
    ...
    for (auto eventIt = room->messageEvents().rbegin(); eventIt < room->messageEvents().rend(); ++eventIt) {
...
{
    return Tp::AvatarSpec({ QStringLiteral("image/png") },
                          0 /* minHeight */,
                          512 /* maxHeight */,
